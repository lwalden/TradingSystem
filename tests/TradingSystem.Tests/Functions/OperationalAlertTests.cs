...
public class OperationalAlertTests
{
    ...
    public async Task RunPreMarket_BrokerConnectFails_SendsOneOrangeConnectFailureAlert()
    {
        ...
        // Exact operator-visible copy: context + consequence + remediation guidance.
        Assert.Contains(
            "Could not connect to the broker for the pre-market run. Options sleeve skipped.",
            body,
    ...
    public async Task EndOfDay_BrokerConnectFails_SendsOneAlert_NoSnapshotNoThrow()
    {
        ...
        // Exact operator-visible copy: context + consequence + remediation guidance.
        Assert.Contains(
            "Could not connect to the broker for the end-of-day run. No snapshot was written.",
            body,
            StringComparison.OrdinalIgnoreCase);
        Assert.Contains(
            "Verify TWS/IB Gateway is running and the API port/client-id match config.",
    ...
    // ---------- 7b. HttpClient timeout cannot escape the no-throw alert contract ----------

    [Fact]
    public async Task OperationalAlert_SendTimesOut_NoThrow_AlertDroppedLogged()
    {
        // An HttpClient.Timeout expiry surfaces as TaskCanceledException with the CALLER's token
        // not cancelled (inner TimeoutException on .NET 5+). It must be swallowed — no throw —
        // and degrade to the same loud, searchable AlertDropped=true terminal log as a transport
        // error, with exception TYPE NAMES only (messages can echo the token-bearing URI).
        var (alerts, handler, logger) = BuildAlertService(
            StubHandler.Throws(new TaskCanceledException(
                "POST https://discord.com/api/webhooks/123/" + SecretToken + " timed out",
                new TimeoutException())));

        await alerts.SendOperationalAlertAsync("Broker Connect Failure — End of Day", "RunId: run-1");

        Assert.Equal(1, handler.InvocationCount); // no retry on timeout — terminal drop
        var logs = AllLogArgStrings(logger).ToList();
        Assert.True(LogCount(logger, LogLevel.Error) >= 1, "timed-out ops alert should stay loud");
        Assert.Contains(logs, s => s.Contains("NOT delivered"));
        Assert.Contains(logs, s => s.Contains("AlertDropped=True"));
        Assert.Contains(logs, s => s.Contains(nameof(TaskCanceledException)));
        Assert.DoesNotContain(logs, s => s.Contains(SecretToken));
        Assert.DoesNotContain(logs, s => s.Contains("/api/webhooks/123"));
    }
