...
DEFAULT_REPORT = REPO_ROOT / "backtests" / "lean" / "results" / "phase1_baseline.json"
GENERATE_SCRIPT = REPO_ROOT / "scripts" / "generate_lean_config.py"
...
def get_expected_hash() -> str | None:
    ...
    """
    for cmd in (["uv", "run", "python", str(GENERATE_SCRIPT), "--hash-only"],
                [sys.executable, str(GENERATE_SCRIPT), "--hash-only"]):
