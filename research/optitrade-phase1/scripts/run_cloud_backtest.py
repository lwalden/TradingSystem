...
CONFIG_JSON      = REPO_ROOT / "backtests" / "lean" / "config.json"
CLOUD_CS         = REPO_ROOT / "backtests" / "QC_CloudBacktest.cs"
STRATEGIES_YAML  = REPO_ROOT / "optimind" / "config" / "strategies.yaml"
RESULTS_DIR      = REPO_ROOT / "backtests" / "lean" / "results"
GENERATE         = REPO_ROOT / "scripts" / "generate_lean_config.py"
PARSE            = REPO_ROOT / "scripts" / "parse_lean_results.py"
EVALUATE         = REPO_ROOT / "scripts" / "evaluate_phase1_gate.py"
...
def step_collect_results(client: QCClient, bt_id: str, bt: dict, run_name: str) -> Path:
    ...
    result = subprocess.run(
        ["uv", "run", "python", str(PARSE), "--log", str(log_path)],
        capture_output=True, text=True,
    )
    if result.returncode != 0:
        print(result.stderr, file=sys.stderr)
